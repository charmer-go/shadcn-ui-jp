...
`Marker` コンポーネントは、ステータス更新、システム メモ、境界線付き行、ラベル付き区切り文字などのインライン会話マーカーを表示します。会話スレッドで [`Message`](/docs/components/message) を使用して作成します。
...
- ステータス テキストをストリーミングするための [`shimmer`](/docs/utils/shimmer) ユーティリティと組み合わせます
