...
`shimmer-spread-<number>` を使用して、間隔スケールを使用してハイライト バンドの幅を設定します。デフォルトは `calc(3ch + 40px)` です。固定ベースにフォント サイズに応じて拡大縮小される `3ch` 用語を加えたものです。
...
ユーザーが動きを減らしたい場合、アニメーションは自動的に無効になり、テキストは通常​​どおり表示されます。設定するものは何もありません。
