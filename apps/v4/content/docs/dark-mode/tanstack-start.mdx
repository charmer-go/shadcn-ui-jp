...
description: TanStack Start アプリにダークモードを追加します。
---
...
TanStack Start は `@tanstack/react-router` の `ScriptOnce` を使用して、React のハイドレーション前に実行されるスクリプトを挿入し、スタイル未適用コンテンツのちらつき（FOUC）を防ぎます。
