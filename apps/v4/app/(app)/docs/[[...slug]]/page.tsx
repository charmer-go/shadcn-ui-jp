...
export async function generateMetadata(props: {
  ...
  const doc = page.data
  const title = getPageDisplayName({ url: page.url, name: doc.title })
...
export default async function Page(props: {
  ...
  const doc = page.data
  const displayTitle = getPageDisplayName({ url: page.url, name: doc.title })
  const MDX = doc.body
          ...
          <div className="mx-auto flex w-full max-w-160 min-w-0 flex-1 flex-col gap-6 px-4 py-6 text-foreground md:px-0 lg:py-8 dark:text-foreground">
            ...
            <div className="hidden h-16 w-full items-center gap-2 px-4 sm:flex sm:px-0">
              {neighbours.previous && (
                ...
                >
                  <Link href={neighbours.previous.url}>
                    <IconArrowLeft /> {getPageDisplayName(neighbours.previous)}
                  </Link>
                </Button>
              )}
              {neighbours.next && (
                ...
                >
                  <Link href={neighbours.next.url}>
                    {getPageDisplayName(neighbours.next)} <IconArrowRight />
                  </Link>
                </Button>
              )}
            </div>
